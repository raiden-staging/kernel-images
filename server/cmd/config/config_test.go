	...
	testCases := []struct {
	...
	}{
		{
			...
			wantCfg: &Config{
				...
				OutputDir:               ".",
				PathToFFmpeg:            "ffmpeg",
				VirtualMediaVideoDevice: "/dev/video42",
				VirtualMediaAudioSink:   "audio_input",
			},
		},
		{
		...
		{
			name: "custom virtual media devices",
			env: map[string]string{
				"VIRTUAL_MEDIA_VIDEO_DEVICE": "/dev/video7",
				"VIRTUAL_MEDIA_AUDIO_SINK":   "custom_sink",
			},
			wantCfg: &Config{
				Port:                    10001,
				FrameRate:               10,
				...
				OutputDir:               ".",
				PathToFFmpeg:            "ffmpeg",
				VirtualMediaVideoDevice: "/dev/video7",
				VirtualMediaAudioSink:   "custom_sink",
			},
		},
		{
