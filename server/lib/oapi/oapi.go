...
// Defines values for VirtualMediaControlRequestTargets.
const (
	Audio VirtualMediaControlRequestTargets = "audio"
	Video VirtualMediaControlRequestTargets = "video"
)

// Defines values for VirtualMediaSourceKind.
const (
	File   VirtualMediaSourceKind = "file"
	Stream VirtualMediaSourceKind = "stream"
)
...
// VirtualMediaControlRequest Target selection for pause/resume operations. Defaults to all active tracks.
type VirtualMediaControlRequest struct {
	Targets *[]VirtualMediaControlRequestTargets `json:"targets,omitempty"`
}

// VirtualMediaControlRequestTargets defines model for VirtualMediaControlRequest.Targets.
type VirtualMediaControlRequestTargets string

// VirtualMediaSource Remote media source to ingest into a virtual device.
type VirtualMediaSource struct {
	// Kind Input type
	Kind VirtualMediaSourceKind `json:"kind"`

	// Loop Loop file inputs. Ignored for streaming inputs.
	Loop *bool `json:"loop,omitempty"`

	// Url Media URL (HLS, RTMP(S), DASH, or direct file URL)
	Url string `json:"url"`
}

// VirtualMediaSourceKind Input type
type VirtualMediaSourceKind string

// VirtualMediaStartRequest Sources to feed into the virtual camera and microphone
type VirtualMediaStartRequest struct {
	// Audio Remote media source to ingest into a virtual device.
	Audio *VirtualMediaSource `json:"audio,omitempty"`

	// Video Remote media source to ingest into a virtual device.
	Video *VirtualMediaSource `json:"video,omitempty"`
}

// VirtualMediaStatus defines model for VirtualMediaStatus.
type VirtualMediaStatus struct {
	Audio *VirtualMediaTrackStatus `json:"audio,omitempty"`

	// ChromiumFlags Chromium flags applied for virtual media
	ChromiumFlags *[]string                `json:"chromium_flags,omitempty"`
	Video         *VirtualMediaTrackStatus `json:"video,omitempty"`
}

// VirtualMediaTrackStatus defines model for VirtualMediaTrackStatus.
type VirtualMediaTrackStatus struct {
	// Active Whether the pipeline is running
	Active bool `json:"active"`

	// LastError Last observed process error, if any
	LastError *string `json:"last_error,omitempty"`

	// OutputPath Output target backing the virtual track (e.g., /dev/video* or pulse sink)
	OutputPath *string `json:"output_path,omitempty"`

	// Paused Whether the pipeline is paused (SIGSTOP)
	Paused bool `json:"paused"`

	// Pid PID of the ffmpeg process driving this track
	Pid *int `json:"pid,omitempty"`

	// Source Remote media source to ingest into a virtual device.
	Source *VirtualMediaSource `json:"source,omitempty"`

	// StartedAt When the pipeline was last started
	StartedAt *time.Time `json:"started_at,omitempty"`
}
...
// PauseVirtualMediaJSONRequestBody defines body for PauseVirtualMedia for application/json ContentType.
type PauseVirtualMediaJSONRequestBody = VirtualMediaControlRequest

// ResumeVirtualMediaJSONRequestBody defines body for ResumeVirtualMedia for application/json ContentType.
type ResumeVirtualMediaJSONRequestBody = VirtualMediaControlRequest

// StartVirtualMediaJSONRequestBody defines body for StartVirtualMedia for application/json ContentType.
type StartVirtualMediaJSONRequestBody = VirtualMediaStartRequest
...
type ClientInterface interface {
	...
	// PauseVirtualMediaWithBody request with any body
	PauseVirtualMediaWithBody(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*http.Response, error)

	PauseVirtualMedia(ctx context.Context, body PauseVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*http.Response, error)

	// ResumeVirtualMediaWithBody request with any body
	ResumeVirtualMediaWithBody(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*http.Response, error)

	ResumeVirtualMedia(ctx context.Context, body ResumeVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*http.Response, error)

	// StartVirtualMediaWithBody request with any body
	StartVirtualMediaWithBody(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*http.Response, error)

	StartVirtualMedia(ctx context.Context, body StartVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*http.Response, error)

	// VirtualMediaStatus request
	VirtualMediaStatus(ctx context.Context, reqEditors ...RequestEditorFn) (*http.Response, error)

	// StopVirtualMedia request
	StopVirtualMedia(ctx context.Context, reqEditors ...RequestEditorFn) (*http.Response, error)
}
...
func (c *Client) PauseVirtualMediaWithBody(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewPauseVirtualMediaRequestWithBody(c.Server, contentType, body)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func (c *Client) PauseVirtualMedia(ctx context.Context, body PauseVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewPauseVirtualMediaRequest(c.Server, body)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func (c *Client) ResumeVirtualMediaWithBody(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewResumeVirtualMediaRequestWithBody(c.Server, contentType, body)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func (c *Client) ResumeVirtualMedia(ctx context.Context, body ResumeVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewResumeVirtualMediaRequest(c.Server, body)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func (c *Client) StartVirtualMediaWithBody(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewStartVirtualMediaRequestWithBody(c.Server, contentType, body)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func (c *Client) StartVirtualMedia(ctx context.Context, body StartVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewStartVirtualMediaRequest(c.Server, body)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func (c *Client) VirtualMediaStatus(ctx context.Context, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewVirtualMediaStatusRequest(c.Server)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
func (c *Client) StopVirtualMedia(ctx context.Context, reqEditors ...RequestEditorFn) (*http.Response, error) {
	req, err := NewStopVirtualMediaRequest(c.Server)
	if err != nil {
		return nil, err
	}
	req = req.WithContext(ctx)
...
// NewPauseVirtualMediaRequest calls the generic PauseVirtualMedia builder with application/json body
func NewPauseVirtualMediaRequest(server string, body PauseVirtualMediaJSONRequestBody) (*http.Request, error) {
	var bodyReader io.Reader
	buf, err := json.Marshal(body)
	if err != nil {
		return nil, err
	...
	bodyReader = bytes.NewReader(buf)
	return NewPauseVirtualMediaRequestWithBody(server, "application/json", bodyReader)
}

// NewPauseVirtualMediaRequestWithBody generates requests for PauseVirtualMedia with any type of body
func NewPauseVirtualMediaRequestWithBody(server string, contentType string, body io.Reader) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/virtual_media/pause")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
// NewResumeVirtualMediaRequest calls the generic ResumeVirtualMedia builder with application/json body
func NewResumeVirtualMediaRequest(server string, body ResumeVirtualMediaJSONRequestBody) (*http.Request, error) {
	var bodyReader io.Reader
	buf, err := json.Marshal(body)
	if err != nil {
		return nil, err
	...
	bodyReader = bytes.NewReader(buf)
	return NewResumeVirtualMediaRequestWithBody(server, "application/json", bodyReader)
}

// NewResumeVirtualMediaRequestWithBody generates requests for ResumeVirtualMedia with any type of body
func NewResumeVirtualMediaRequestWithBody(server string, contentType string, body io.Reader) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/virtual_media/resume")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
// NewStartVirtualMediaRequest calls the generic StartVirtualMedia builder with application/json body
func NewStartVirtualMediaRequest(server string, body StartVirtualMediaJSONRequestBody) (*http.Request, error) {
	var bodyReader io.Reader
	buf, err := json.Marshal(body)
	if err != nil {
		return nil, err
	...
	bodyReader = bytes.NewReader(buf)
	return NewStartVirtualMediaRequestWithBody(server, "application/json", bodyReader)
}

// NewStartVirtualMediaRequestWithBody generates requests for StartVirtualMedia with any type of body
func NewStartVirtualMediaRequestWithBody(server string, contentType string, body io.Reader) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/virtual_media/start")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
// NewVirtualMediaStatusRequest generates requests for VirtualMediaStatus
func NewVirtualMediaStatusRequest(server string) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/virtual_media/status")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
// NewStopVirtualMediaRequest generates requests for StopVirtualMedia
func NewStopVirtualMediaRequest(server string) (*http.Request, error) {
	var err error
	...
	operationPath := fmt.Sprintf("/virtual_media/stop")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
...
type ClientWithResponsesInterface interface {
	...
	// PauseVirtualMediaWithBodyWithResponse request with any body
	PauseVirtualMediaWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*PauseVirtualMediaResponse, error)

	PauseVirtualMediaWithResponse(ctx context.Context, body PauseVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*PauseVirtualMediaResponse, error)

	// ResumeVirtualMediaWithBodyWithResponse request with any body
	ResumeVirtualMediaWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*ResumeVirtualMediaResponse, error)

	ResumeVirtualMediaWithResponse(ctx context.Context, body ResumeVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*ResumeVirtualMediaResponse, error)

	// StartVirtualMediaWithBodyWithResponse request with any body
	StartVirtualMediaWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*StartVirtualMediaResponse, error)

	StartVirtualMediaWithResponse(ctx context.Context, body StartVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*StartVirtualMediaResponse, error)

	// VirtualMediaStatusWithResponse request
	VirtualMediaStatusWithResponse(ctx context.Context, reqEditors ...RequestEditorFn) (*VirtualMediaStatusResponse, error)

	// StopVirtualMediaWithResponse request
	StopVirtualMediaWithResponse(ctx context.Context, reqEditors ...RequestEditorFn) (*StopVirtualMediaResponse, error)
}
...
type PauseVirtualMediaResponse struct {
	Body         []byte
	HTTPResponse *http.Response
	JSON200      *VirtualMediaStatus
	JSON400      *BadRequestError
	JSON500      *InternalError
}

// Status returns HTTPResponse.Status
func (r PauseVirtualMediaResponse) Status() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Status
	}
	return http.StatusText(0)
...
// StatusCode returns HTTPResponse.StatusCode
func (r PauseVirtualMediaResponse) StatusCode() int {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.StatusCode
	}
	return 0
...
type ResumeVirtualMediaResponse struct {
	Body         []byte
	HTTPResponse *http.Response
	JSON200      *VirtualMediaStatus
	JSON400      *BadRequestError
...
// Status returns HTTPResponse.Status
func (r ResumeVirtualMediaResponse) Status() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Status
	}
	return http.StatusText(0)
...
// StatusCode returns HTTPResponse.StatusCode
func (r ResumeVirtualMediaResponse) StatusCode() int {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.StatusCode
	}
	return 0
...
type StartVirtualMediaResponse struct {
	Body         []byte
	HTTPResponse *http.Response
	JSON200      *VirtualMediaStatus
	JSON400      *BadRequestError
...
// Status returns HTTPResponse.Status
func (r StartVirtualMediaResponse) Status() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Status
	}
	return http.StatusText(0)
...
// StatusCode returns HTTPResponse.StatusCode
func (r StartVirtualMediaResponse) StatusCode() int {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.StatusCode
	}
	return 0
...
type VirtualMediaStatusResponse struct {
	Body         []byte
	HTTPResponse *http.Response
	JSON200      *VirtualMediaStatus
	JSON500      *InternalError
...
// Status returns HTTPResponse.Status
func (r VirtualMediaStatusResponse) Status() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Status
	}
	return http.StatusText(0)
...
// StatusCode returns HTTPResponse.StatusCode
func (r VirtualMediaStatusResponse) StatusCode() int {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.StatusCode
	}
	return 0
...
type StopVirtualMediaResponse struct {
	Body         []byte
	HTTPResponse *http.Response
	JSON200      *VirtualMediaStatus
	JSON500      *InternalError
...
// Status returns HTTPResponse.Status
func (r StopVirtualMediaResponse) Status() string {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.Status
	}
	return http.StatusText(0)
...
// StatusCode returns HTTPResponse.StatusCode
func (r StopVirtualMediaResponse) StatusCode() int {
	if r.HTTPResponse != nil {
		return r.HTTPResponse.StatusCode
	}
	return 0
...
// PauseVirtualMediaWithBodyWithResponse request with arbitrary body returning *PauseVirtualMediaResponse
func (c *ClientWithResponses) PauseVirtualMediaWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*PauseVirtualMediaResponse, error) {
	rsp, err := c.PauseVirtualMediaWithBody(ctx, contentType, body, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParsePauseVirtualMediaResponse(rsp)
}

func (c *ClientWithResponses) PauseVirtualMediaWithResponse(ctx context.Context, body PauseVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*PauseVirtualMediaResponse, error) {
	rsp, err := c.PauseVirtualMedia(ctx, body, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParsePauseVirtualMediaResponse(rsp)
...
// ResumeVirtualMediaWithBodyWithResponse request with arbitrary body returning *ResumeVirtualMediaResponse
func (c *ClientWithResponses) ResumeVirtualMediaWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*ResumeVirtualMediaResponse, error) {
	rsp, err := c.ResumeVirtualMediaWithBody(ctx, contentType, body, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseResumeVirtualMediaResponse(rsp)
}

func (c *ClientWithResponses) ResumeVirtualMediaWithResponse(ctx context.Context, body ResumeVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*ResumeVirtualMediaResponse, error) {
	rsp, err := c.ResumeVirtualMedia(ctx, body, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseResumeVirtualMediaResponse(rsp)
...
// StartVirtualMediaWithBodyWithResponse request with arbitrary body returning *StartVirtualMediaResponse
func (c *ClientWithResponses) StartVirtualMediaWithBodyWithResponse(ctx context.Context, contentType string, body io.Reader, reqEditors ...RequestEditorFn) (*StartVirtualMediaResponse, error) {
	rsp, err := c.StartVirtualMediaWithBody(ctx, contentType, body, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseStartVirtualMediaResponse(rsp)
}

func (c *ClientWithResponses) StartVirtualMediaWithResponse(ctx context.Context, body StartVirtualMediaJSONRequestBody, reqEditors ...RequestEditorFn) (*StartVirtualMediaResponse, error) {
	rsp, err := c.StartVirtualMedia(ctx, body, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseStartVirtualMediaResponse(rsp)
...
// VirtualMediaStatusWithResponse request returning *VirtualMediaStatusResponse
func (c *ClientWithResponses) VirtualMediaStatusWithResponse(ctx context.Context, reqEditors ...RequestEditorFn) (*VirtualMediaStatusResponse, error) {
	rsp, err := c.VirtualMediaStatus(ctx, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseVirtualMediaStatusResponse(rsp)
}

// StopVirtualMediaWithResponse request returning *StopVirtualMediaResponse
func (c *ClientWithResponses) StopVirtualMediaWithResponse(ctx context.Context, reqEditors ...RequestEditorFn) (*StopVirtualMediaResponse, error) {
	rsp, err := c.StopVirtualMedia(ctx, reqEditors...)
	if err != nil {
		return nil, err
	}
	return ParseStopVirtualMediaResponse(rsp)
}
...
// ParsePauseVirtualMediaResponse parses an HTTP response from a PauseVirtualMediaWithResponse call
func ParsePauseVirtualMediaResponse(rsp *http.Response) (*PauseVirtualMediaResponse, error) {
	bodyBytes, err := io.ReadAll(rsp.Body)
	defer func() { _ = rsp.Body.Close() }()
	if err != nil {
		return nil, err
	...
	response := &PauseVirtualMediaResponse{
		Body:         bodyBytes,
		HTTPResponse: rsp,
	}
...
// ParseResumeVirtualMediaResponse parses an HTTP response from a ResumeVirtualMediaWithResponse call
func ParseResumeVirtualMediaResponse(rsp *http.Response) (*ResumeVirtualMediaResponse, error) {
	bodyBytes, err := io.ReadAll(rsp.Body)
	defer func() { _ = rsp.Body.Close() }()
	if err != nil {
		return nil, err
	...
	response := &ResumeVirtualMediaResponse{
		Body:         bodyBytes,
		HTTPResponse: rsp,
	}
...
// ParseStartVirtualMediaResponse parses an HTTP response from a StartVirtualMediaWithResponse call
func ParseStartVirtualMediaResponse(rsp *http.Response) (*StartVirtualMediaResponse, error) {
	bodyBytes, err := io.ReadAll(rsp.Body)
	defer func() { _ = rsp.Body.Close() }()
	if err != nil {
		return nil, err
	...
	response := &StartVirtualMediaResponse{
		Body:         bodyBytes,
		HTTPResponse: rsp,
	}
...
// ParseVirtualMediaStatusResponse parses an HTTP response from a VirtualMediaStatusWithResponse call
func ParseVirtualMediaStatusResponse(rsp *http.Response) (*VirtualMediaStatusResponse, error) {
	bodyBytes, err := io.ReadAll(rsp.Body)
	defer func() { _ = rsp.Body.Close() }()
	if err != nil {
		return nil, err
	...
	response := &VirtualMediaStatusResponse{
		Body:         bodyBytes,
		HTTPResponse: rsp,
	}
...
// ParseStopVirtualMediaResponse parses an HTTP response from a StopVirtualMediaWithResponse call
func ParseStopVirtualMediaResponse(rsp *http.Response) (*StopVirtualMediaResponse, error) {
	bodyBytes, err := io.ReadAll(rsp.Body)
	defer func() { _ = rsp.Body.Close() }()
	if err != nil {
		return nil, err
	...
	response := &StopVirtualMediaResponse{
		Body:         bodyBytes,
		HTTPResponse: rsp,
	}
...
type ServerInterface interface {
	...
	// Pause virtual media streams without removing them
	// (POST /virtual_media/pause)
	PauseVirtualMedia(w http.ResponseWriter, r *http.Request)
	// Resume previously paused virtual media streams
	// (POST /virtual_media/resume)
	ResumeVirtualMedia(w http.ResponseWriter, r *http.Request)
	// Configure virtual camera and microphone inputs
	// (POST /virtual_media/start)
	StartVirtualMedia(w http.ResponseWriter, r *http.Request)
	// Get current virtual media pipeline status
	// (GET /virtual_media/status)
	VirtualMediaStatus(w http.ResponseWriter, r *http.Request)
	// Stop all virtual media inputs and clear Chromium flags
	// (POST /virtual_media/stop)
	StopVirtualMedia(w http.ResponseWriter, r *http.Request)
}
...
// Pause virtual media streams without removing them
// (POST /virtual_media/pause)
func (_ Unimplemented) PauseVirtualMedia(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Resume previously paused virtual media streams
// (POST /virtual_media/resume)
func (_ Unimplemented) ResumeVirtualMedia(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Configure virtual camera and microphone inputs
// (POST /virtual_media/start)
func (_ Unimplemented) StartVirtualMedia(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Get current virtual media pipeline status
// (GET /virtual_media/status)
func (_ Unimplemented) VirtualMediaStatus(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusNotImplemented)
}

// Stop all virtual media inputs and clear Chromium flags
// (POST /virtual_media/stop)
func (_ Unimplemented) StopVirtualMedia(w http.ResponseWriter, r *http.Request) {
	w.WriteHeader(http.StatusNotImplemented)
}
...
// PauseVirtualMedia operation middleware
func (siw *ServerInterfaceWrapper) PauseVirtualMedia(w http.ResponseWriter, r *http.Request) {

	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.PauseVirtualMedia(w, r)
	}))
...
// ResumeVirtualMedia operation middleware
func (siw *ServerInterfaceWrapper) ResumeVirtualMedia(w http.ResponseWriter, r *http.Request) {

	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.ResumeVirtualMedia(w, r)
	}))
...
// StartVirtualMedia operation middleware
func (siw *ServerInterfaceWrapper) StartVirtualMedia(w http.ResponseWriter, r *http.Request) {

	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.StartVirtualMedia(w, r)
	}))
...
// VirtualMediaStatus operation middleware
func (siw *ServerInterfaceWrapper) VirtualMediaStatus(w http.ResponseWriter, r *http.Request) {

	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.VirtualMediaStatus(w, r)
	}))
...
// StopVirtualMedia operation middleware
func (siw *ServerInterfaceWrapper) StopVirtualMedia(w http.ResponseWriter, r *http.Request) {

	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.StopVirtualMedia(w, r)
	}))
...
func HandlerWithOptions(si ServerInterface, options ChiServerOptions) http.Handler {
	...
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/virtual_media/pause", wrapper.PauseVirtualMedia)
	})
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/virtual_media/resume", wrapper.ResumeVirtualMedia)
	})
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/virtual_media/start", wrapper.StartVirtualMedia)
	})
	r.Group(func(r chi.Router) {
		r.Get(options.BaseURL+"/virtual_media/status", wrapper.VirtualMediaStatus)
	})
	r.Group(func(r chi.Router) {
		r.Post(options.BaseURL+"/virtual_media/stop", wrapper.StopVirtualMedia)
	})
...
type PauseVirtualMediaRequestObject struct {
	Body *PauseVirtualMediaJSONRequestBody
}

type PauseVirtualMediaResponseObject interface {
	VisitPauseVirtualMediaResponse(w http.ResponseWriter) error
}

type PauseVirtualMedia200JSONResponse VirtualMediaStatus

func (response PauseVirtualMedia200JSONResponse) VisitPauseVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type PauseVirtualMedia400JSONResponse struct{ BadRequestErrorJSONResponse }

func (response PauseVirtualMedia400JSONResponse) VisitPauseVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(400)
...
type PauseVirtualMedia500JSONResponse struct{ InternalErrorJSONResponse }

func (response PauseVirtualMedia500JSONResponse) VisitPauseVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type ResumeVirtualMediaRequestObject struct {
	Body *ResumeVirtualMediaJSONRequestBody
}

type ResumeVirtualMediaResponseObject interface {
	VisitResumeVirtualMediaResponse(w http.ResponseWriter) error
}

type ResumeVirtualMedia200JSONResponse VirtualMediaStatus

func (response ResumeVirtualMedia200JSONResponse) VisitResumeVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type ResumeVirtualMedia400JSONResponse struct{ BadRequestErrorJSONResponse }

func (response ResumeVirtualMedia400JSONResponse) VisitResumeVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(400)
...
type ResumeVirtualMedia500JSONResponse struct{ InternalErrorJSONResponse }

func (response ResumeVirtualMedia500JSONResponse) VisitResumeVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type StartVirtualMediaRequestObject struct {
	Body *StartVirtualMediaJSONRequestBody
}

type StartVirtualMediaResponseObject interface {
	VisitStartVirtualMediaResponse(w http.ResponseWriter) error
}

type StartVirtualMedia200JSONResponse VirtualMediaStatus

func (response StartVirtualMedia200JSONResponse) VisitStartVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type StartVirtualMedia400JSONResponse struct{ BadRequestErrorJSONResponse }

func (response StartVirtualMedia400JSONResponse) VisitStartVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(400)
...
type StartVirtualMedia500JSONResponse struct{ InternalErrorJSONResponse }

func (response StartVirtualMedia500JSONResponse) VisitStartVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type VirtualMediaStatusRequestObject struct {
}

type VirtualMediaStatusResponseObject interface {
	VisitVirtualMediaStatusResponse(w http.ResponseWriter) error
}

type VirtualMediaStatus200JSONResponse VirtualMediaStatus

func (response VirtualMediaStatus200JSONResponse) VisitVirtualMediaStatusResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type VirtualMediaStatus500JSONResponse struct{ InternalErrorJSONResponse }

func (response VirtualMediaStatus500JSONResponse) VisitVirtualMediaStatusResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type StopVirtualMediaRequestObject struct {
}

type StopVirtualMediaResponseObject interface {
	VisitStopVirtualMediaResponse(w http.ResponseWriter) error
}

type StopVirtualMedia200JSONResponse VirtualMediaStatus

func (response StopVirtualMedia200JSONResponse) VisitStopVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
...
type StopVirtualMedia500JSONResponse struct{ InternalErrorJSONResponse }

func (response StopVirtualMedia500JSONResponse) VisitStopVirtualMediaResponse(w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(500)
...
type StrictServerInterface interface {
	...
	// Pause virtual media streams without removing them
	// (POST /virtual_media/pause)
	PauseVirtualMedia(ctx context.Context, request PauseVirtualMediaRequestObject) (PauseVirtualMediaResponseObject, error)
	// Resume previously paused virtual media streams
	// (POST /virtual_media/resume)
	ResumeVirtualMedia(ctx context.Context, request ResumeVirtualMediaRequestObject) (ResumeVirtualMediaResponseObject, error)
	// Configure virtual camera and microphone inputs
	// (POST /virtual_media/start)
	StartVirtualMedia(ctx context.Context, request StartVirtualMediaRequestObject) (StartVirtualMediaResponseObject, error)
	// Get current virtual media pipeline status
	// (GET /virtual_media/status)
	VirtualMediaStatus(ctx context.Context, request VirtualMediaStatusRequestObject) (VirtualMediaStatusResponseObject, error)
	// Stop all virtual media inputs and clear Chromium flags
	// (POST /virtual_media/stop)
	StopVirtualMedia(ctx context.Context, request StopVirtualMediaRequestObject) (StopVirtualMediaResponseObject, error)
}
...
// PauseVirtualMedia operation middleware
func (sh *strictHandler) PauseVirtualMedia(w http.ResponseWriter, r *http.Request) {
	var request PauseVirtualMediaRequestObject

	var body PauseVirtualMediaJSONRequestBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		sh.options.RequestErrorHandlerFunc(w, r, fmt.Errorf("can't decode JSON body: %w", err))
		return
	}
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.PauseVirtualMedia(ctx, request.(PauseVirtualMediaRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "PauseVirtualMedia")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(PauseVirtualMediaResponseObject); ok {
		if err := validResponse.VisitPauseVirtualMediaResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// ResumeVirtualMedia operation middleware
func (sh *strictHandler) ResumeVirtualMedia(w http.ResponseWriter, r *http.Request) {
	var request ResumeVirtualMediaRequestObject

	var body ResumeVirtualMediaJSONRequestBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		sh.options.RequestErrorHandlerFunc(w, r, fmt.Errorf("can't decode JSON body: %w", err))
		return
	}
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.ResumeVirtualMedia(ctx, request.(ResumeVirtualMediaRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "ResumeVirtualMedia")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(ResumeVirtualMediaResponseObject); ok {
		if err := validResponse.VisitResumeVirtualMediaResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// StartVirtualMedia operation middleware
func (sh *strictHandler) StartVirtualMedia(w http.ResponseWriter, r *http.Request) {
	var request StartVirtualMediaRequestObject

	var body StartVirtualMediaJSONRequestBody
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		sh.options.RequestErrorHandlerFunc(w, r, fmt.Errorf("can't decode JSON body: %w", err))
		return
	}
	...
	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.StartVirtualMedia(ctx, request.(StartVirtualMediaRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "StartVirtualMedia")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(StartVirtualMediaResponseObject); ok {
		if err := validResponse.VisitStartVirtualMediaResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// VirtualMediaStatus operation middleware
func (sh *strictHandler) VirtualMediaStatus(w http.ResponseWriter, r *http.Request) {
	var request VirtualMediaStatusRequestObject

	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.VirtualMediaStatus(ctx, request.(VirtualMediaStatusRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "VirtualMediaStatus")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(VirtualMediaStatusResponseObject); ok {
		if err := validResponse.VisitVirtualMediaStatusResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
// StopVirtualMedia operation middleware
func (sh *strictHandler) StopVirtualMedia(w http.ResponseWriter, r *http.Request) {
	var request StopVirtualMediaRequestObject

	handler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, request interface{}) (interface{}, error) {
		return sh.ssi.StopVirtualMedia(ctx, request.(StopVirtualMediaRequestObject))
	}
	for _, middleware := range sh.middlewares {
		handler = middleware(handler, "StopVirtualMedia")
	}
	...
	if err != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, err)
	} else if validResponse, ok := response.(StopVirtualMediaResponseObject); ok {
		if err := validResponse.VisitStopVirtualMediaResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
	} else if response != nil {
		sh.options.ResponseErrorHandlerFunc(w, r, fmt.Errorf("unexpected response type: %T", response))
...
var swaggerSpec = []string{
	...
	"H4sIAAAAAAAC/+x9aXPbOLboX0HxTVXsO1qcrafa8yltO91+STouy5me1608XYg8knBNAmwAlKyk/N9v",
	"4QDcRFCb7TjumqqpaUckgQOcfcHB1yAUSSo4cK2C46+BBJUKrgD/8RONLuHPDJQ+k1JI81MouAauzZ80",
	"TWMWUs0E7/+PEtz8psIZJNT89TcJk+A4+D/9cvy+far6drTb29tOEIEKJUvNIMGxmZC4GYPbTnAi+CRm",
	"4beaPZ/OTH3ONUhO4280dT4dGYCcgyTuxU7wq9BvRcajbwTHr0ITnC8wz9zrZrSTmIXXH0SmIMePASCK",
	"mPmQxhdSpCA1M3QzobGCTpBWfvoajDOtLYT1CXFIYp8SLQgzG0FDTRZMz4JOADxLguM/ghgmOugEkk1n",
	"5r8Ji6IYgk4wpuF10AkmQi6ojILPnUAvUwiOA6Ul41OzhaEBfWR/Xp3+apkCEROC7xAa4s/lrJFYmH9m",
	"aeCG8U4wE3E0uoal8i0vYhMGkpjHZn3mXRJl5lOiZ2AnDjoB05Dg943R3Q9USro0/+ZZMsKv3HQTmsU6",
	"OH7eQGWWjEGaxWmWAE4uIQWqa/O60c22TwEp7qa5in+TUAgZMU417lYxAEmFYm7PmiMtmyP9v31Guu0E",
	"Ev7MmITIIOUmMEOXiBDj/wHLtCcSqIZTJiHUQi73o9RERB5C+Zjaz0mUj07Mi+RAhJrGxKKrQ6A37ZF/",
	"vH592COnFjO48f94/boXdIKUasPmwXHw//846v7j89eXnVe3fws8JJVSPWsC8WasRJxpqABhXjQzhLj0",
	"lUn6vf9qDr6ymziTbzNPIQYNF1TP9tvHDUvIAY9wmvsH/BJCJLTpftCzqAn7eQRcW3Z2pCvzSSorIW/i",
