...
Endpoints live under `/input/devices/virtual/*`. These examples use the sample media provided in the task docs and assume the API is reachable on `http://localhost:444`.
...
# Expect: 200 JSON, state=paused, mode=virtual-file, video_file/audio_file paths under /tmp/virtual-inputs, Chromium flags updated
```
...
# Expect: 200 JSON, state=paused, live ffmpeg terminated (or replaced by black/silence only)
```
